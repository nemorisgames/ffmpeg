...
@app.get("/health")
def health():
    # Basic health check for n8n or EasyPanel
    return {"status": "ok"}
...
async def render_video(
    ...
    # Create a unique folder for this render job
    ...
    try:
        ...
        # Save uploaded subtitles file
        with open(subtitles_path, "wb") as buffer:
            shutil.copyfileobj(subtitles.file, buffer)

        # FFmpeg burns subtitles into the video and replaces the audio track
        command = [
            "ffmpeg",
            "-y",
            "-i", str(video_path),
